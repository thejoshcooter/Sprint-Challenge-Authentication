...
Sessions allow us to persist data and authorization for a set interval of time. Using sessions eliminates the need for a manual verification on every request sent by the client.  


1. What does bcrypt do to help us store passwords in a secure manner.  

The bcrypt library allows us to create secure password hashes for transmission to and from the server to avoid any plain text passwords being sent over the wire.  


1. What does bcrypt do to slow down attackers?  

The bcrypt library also allows us to create hashing rounds where the initial password is rehashed over and over again for a predetermined number of rounds - this means that a potential attacker would not only need to determine all potential hashes ahead of time, but they would also need to know and execute the number of hashing rounds.  
